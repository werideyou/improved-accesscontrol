...
/** Function: efIACAccessControlShowSearchHitTitle( &$title, &$text, $result,
			$terms, $page )
 * Hook: ShowSearchHitTitle
 * Hide search results if the page is unauthorized, using a blunt method of invalidating
 *	the result and blanking out the text before it's rendered.
 * &$title --> Title object for article being linked to (will be removed)
 * &$text --> Text to display for the link (will become blank)
 * $result --> Search result
 * $terms --> Terms searched for
 * $page --> Search results page
 * Returns: Whether this search result should be shown
 */
function efIACAccessControlShowSearchHitTitle( &$title, &$text, $result, $terms, $page ) {
	$canRead = $title->userCan( 'read' );
	if (!$canRead) {
	    efIACDebugLog( "(efIACAccessControlShowSearchHitTitle) hiding search result");
		$title = null;
		$text = "";
	}
	return $canRead;
